...
function fresnel2(Uin::MonoLightField2D, d::Real)
    Uout = fresnel2(Uin.distribution_data, uval(d), uval(Uin.wavelength), (uval.(Uin.size))...)
...
function fresnel1(Uin::MonoLightField2D, d::Real)
    (Uout, Lout) = fresnel1(Uin.distribution_data, uval(d), uval(Uin.wavelength), (uval.(Uin.size))...)
    MonoLightField2D(Uin, distribution_data=Uout, size=Lout)
end
